        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('pajakrestoran.index', []) }}" class="btn btn-sm btn-primary">
...
@include('pajak.restoran.modal-laporan')
@endsection
...
<script>
    ...
    $(document).ready(function() {
        var restoranId = $("#Datalaporan").data("id");
        loadLaporan(restoranId);
    });

    function loadLaporan(id, page = 1) {
        $.ajax({
            url: "{{ route('laporan.restoran.data', ':id') }}".replace(':id', id) + "?page=" + page,
            type: "GET",
...
<script>
    $(document).ready(function () {
        $("#formTambah").submit(function (e) {
            ...
            let id = $("#Datalaporan").data("id"); // ID Pajak Restoran
            formData.append("id", id);

            $.ajax({
                url: "{{ route('laporan.restoran.store') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
...
<script>
    ...
    $('#formEditPemberitahuan').on('submit', function (e) {
        ...
        $.ajax({
            url: "{{ route('laporan.restoran.pemberitahuan', ':id') }}".replace(':id', id),
            type: "PUT",
            data: {
...
<script>
    $(document).on('click', '.btn-tooltips', function () {
        ...
        // Ambil data berdasarkan ID (AJAX GET)
        $.ajax({
            url: "{{ route('laporan.restoran.teguran.show', ':id') }}".replace(':id', id),
            type: "GET",
            success: function (response) {
                if (response.success) {
    ...
    $('#formEditTeguran').on('submit', function (e) {  // Nama form harus benar
        ...
        $.ajax({
            url: "{{ route('laporan.restoran.teguran', ':id') }}".replace(':id', id),
            type: "PUT",
            data: {
