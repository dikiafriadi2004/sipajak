        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-end">
                    <a href="{{ route('pajakhotel.index', []) }}" class="btn btn-sm btn-primary">
...
@include('pajak.hotel.modal-laporan')
@endsection
...
<script>
    ...
    $(document).ready(function() {
        var hotelId = $("#Datalaporan").data("id");
        loadLaporan(hotelId);
    });

    function loadLaporan(id, page = 1) {
        $.ajax({
            url: "{{ route('laporan.hotel.data', ':id') }}".replace(':id', id) + "?page=" + page,
            type: "GET",
    ...
    $(document).on("click", ".pagination a", function (event) {
        ...
        let id = $("#Datalaporan").data("id"); // Ambil ID hotel
...
<script>
    $(document).ready(function () {
        function loadLaporan() {
            ...
            $.ajax({
                url: "{{ route('laporan.hotel.filter', ':id') }}".replace(':id', id),
                method: "GET",
...
<script>
    $(document).ready(function () {
        $("#formTambah").submit(function (e) {
            ...
            let id = $("#Datalaporan").data("id"); // ID Pajak Hotel
            formData.append("id", id);

            $.ajax({
                url: "{{ route('laporan.hotel.store') }}",
                type: "POST",
                data: formData,
                contentType: false,
                processData: false,
...
<script>
    $(document).on('click', '.btn-tooltips', function () {
        ...
        $.ajax({
            url: "{{ route('laporan.hotel.pemberitahuan.show', '') }}/" + id,
            type: "GET",
            success: function (response) {
                if (response.success) {
    ...
    $('#formEditPemberitahuan').on('submit', function (e) {
        ...
        $.ajax({
            url: "{{ route('laporan.hotel.pemberitahuan', ':id') }}".replace(':id', id),
            type: "PUT",
            data: {
...
<script>
    $(document).on('click', '.btn-tooltips', function () {
        ...
        // Ambil data berdasarkan ID (AJAX GET)
        $.ajax({
            url: "{{ route('laporan.hotel.teguran.show', ':id') }}".replace(':id', id),
            type: "GET",
            success: function (response) {
                if (response.success) {
    ...
    $('#formEditTeguran').on('submit', function (e) {  // Nama form harus benar
        ...
        $.ajax({
            url: "{{ route('laporan.hotel.teguran', ':id') }}".replace(':id', id),
            type: "PUT",
            data: {
