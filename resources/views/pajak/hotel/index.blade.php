        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <h2 class="fw-bold">Pajak Hotel</h2>
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <nav aria-label="Page navigation">
                            <ul class="pagination">
                                {{-- Tombol Previous --}}
                                @if ($pajakhotel->onFirstPage())
                                    <li class="page-item disabled"><a class="page-link">Previous</a></li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhotel->currentPage() - 1 }}">Previous</a>
                                    </li>
                                @endif

                                {{-- Halaman Pertama (Selalu Ditampilkan) --}}
                                @if ($pajakhotel->currentPage() > 2)
                                    <li class="page-item">
                                        <a class="page-link" href="#" data-page="1">1</a>
                                    </li>
                                    @if ($pajakhotel->currentPage() > 3)
                                        <li class="page-item disabled"><a class="page-link">...</a></li>
                                    @endif
                                @endif

                                {{-- Halaman Sebelum, Saat Ini, dan Sesudah --}}
                                @for ($i = max(1, $pajakhotel->currentPage() - 1); $i <= min($pajakhotel->lastPage(), $pajakhotel->currentPage() + 1); $i++)
                                    <li class="page-item {{ $pajakhotel->currentPage() == $i ? 'active' : '' }}">
                                        <a class="page-link" href="#"
                                            data-page="{{ $i }}">{{ $i }}</a>
                                    </li>
                                @endfor
                                ...
                                @if ($pajakhotel->currentPage() < $pajakhotel->lastPage() - 1)
                                    @if ($pajakhotel->currentPage() < $pajakhotel->lastPage() - 2)
                                        <li class="page-item disabled"><a class="page-link">...</a></li>
                                    @endif
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhotel->lastPage() }}">{{ $pajakhotel->lastPage() }}</a>
                                    </li>
                                @endif

                                {{-- Tombol Next --}}
                                @if ($pajakhotel->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhotel->currentPage() + 1 }}">Next</a>
                                    </li>
                                @else
    ...
    <script>
        ...
        function loadData(page = 1) {
            $.ajax({
                ...
                url: "{{ route('pajakhotel.data') }}?page=" + page,
                type: "GET",
    ...
    <script>
        $(document).on('click', '.btn-delete', function() {
            ...
            if (confirm("Apakah kamu yakin ingin menghapus data ini?")) {
                $.ajax({
                    url: "{{ route('pajakhotel.destroy', ':id') }}".replace(':id', id),
                    type: "DELETE",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.ajax({
                    url: "{{ route('pajakhotel.edit', ':id') }}".replace(':id', id),
                    type: "GET",
                    success: function(response) {
                        if (response.success) {
            ...
            $('#formEdit').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('pajakhotel.update', ':id') }}".replace(':id', id),
                    type: "PUT",
                    data: formData,
                    success: function(response) {
                        if (response.success) {
