        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <h2 class="fw-bold">Pajak Hiburan</h2>
                    <div class="d-flex flex-wrap justify-content-end gap-3 mb-3">
                        ...
                        <div>
                            <a href="#" class="btn btn-sm btn-primary btn-tambah" data-bs-toggle="modal"
                                data-bs-target="#modalTambah" data-bs-original-title="Tambah Data">
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <nav aria-label="Page navigation">
                            <ul class="pagination">
                                {{-- Tombol Previous --}}
                                @if ($pajakhiburan->onFirstPage())
                                    <li class="page-item disabled"><a class="page-link">Previous</a></li>
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhiburan->currentPage() - 1 }}">Previous</a>
                                    </li>
                                @endif

                                {{-- Halaman Pertama (Selalu Ditampilkan) --}}
                                @if ($pajakhiburan->currentPage() > 2)
                                    <li class="page-item">
                                        <a class="page-link" href="#" data-page="1">1</a>
                                    </li>
                                    @if ($pajakhiburan->currentPage() > 3)
                                        <li class="page-item disabled"><a class="page-link">...</a></li>
                                    @endif
                                @endif

                                {{-- Halaman Sebelum, Saat Ini, dan Sesudah --}}
                                @for ($i = max(1, $pajakhiburan->currentPage() - 1); $i <= min($pajakhiburan->lastPage(), $pajakhiburan->currentPage() + 1); $i++)
                                    <li class="page-item {{ $pajakhiburan->currentPage() == $i ? 'active' : '' }}">
                                        <a class="page-link" href="#"
                                            data-page="{{ $i }}">{{ $i }}</a>
                                    </li>
                                @endfor
                                ...
                                @if ($pajakhiburan->currentPage() < $pajakhiburan->lastPage() - 1)
                                    @if ($pajakhiburan->currentPage() < $pajakhiburan->lastPage() - 2)
                                        <li class="page-item disabled"><a class="page-link">...</a></li>
                                    @endif
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhiburan->lastPage() }}">{{ $pajakhiburan->lastPage() }}</a>
                                    </li>
                                @endif

                                {{-- Tombol Next --}}
                                @if ($pajakhiburan->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="#"
                                            data-page="{{ $pajakhiburan->currentPage() + 1 }}">Next</a>
                                    </li>
                                @else
    ...
    <script>
        ...
        function loadData(page = 1) {
            $.ajax({
                ...
                url: "{{ route('pajakhiburan.data') }}?page=" + page,
                type: "GET",
        ...
        $(document).ready(function() {
            ...
            function loadData(page = 1, search = '') {
                $.ajax({
                    ...
                    error: function() {
                        $("#DataTable").html(
                            '<p class="text-center text-danger">Gagal mengambil data</p>');
                    }
                });
            }
        });
    ...
    <script>
        $(document).on('click', '.btn-delete', function() {
            ...
            if (confirm("Apakah kamu yakin ingin menghapus data ini?")) {
                $.ajax({
                    url: "{{ route('pajakhiburan.destroy', ':id') }}".replace(':id', id),
                    type: "DELETE",
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-edit', function() {
                ...
                $.ajax({
                    url: "{{ route('pajakhiburan.edit', ':id') }}".replace(':id', id),
                    type: "GET",
                    success: function(response) {
                        if (response.success) {
            ...
            $('#formEdit').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('pajakhiburan.update', ':id') }}".replace(':id', id),
                    type: "PUT",
                    data: formData,
                    success: function(response) {
                        if (response.success) {
